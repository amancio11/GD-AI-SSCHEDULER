...
#   → { entries: [...], dependencies: [...], rp_markers: [...] }
...
async def get_enriched_gantt(
    ...
    #    Vertici = entries, archi = (a → b) se end(a) <= start(b) ed esiste vincolo
    #    Per semplicità qui usiamo solo gli archi RP DAG (lo stesso che genera
    #    `dependencies` sotto). Per il critical path "vero" servirebbe il modello
    #    di scheduling completo — questo è una buona approssimazione visiva.
    ...
    from app.db.models import MachineOrder
    ...
    # 6) Mappa: production_order_id → set di entries del sottoalbero BOM
    ...
    # Mappa material_code → order_id (per target_order_material dei RP)
    material_to_id: dict[str, UUID] = {
        o.material_code: o.id for o in all_orders
    }

    # Mappa: entry_id → operation_id → production_order_id
