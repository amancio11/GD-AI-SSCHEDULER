...
os.environ['CPSAT_TIMEOUT_SECONDS'] = '30'
print('ENV_CPSAT', os.getenv('CPSAT_TIMEOUT_SECONDS'))
...
try:
    ...
    print('operators', len(qual_ops), 'slots', sum(len(o.available_slots) for o in qual_ops))
    builder = CpsatModelBuilder(sched_ops, qual_ops, 50399, epoch, {}, [])
    print('builder timeout', builder.TIMEOUT)
    t0 = time.time()
    sol = builder.build_and_solve('FINISH_BY_DATE', {})
    t1 = time.time()
    print('status', sol.status, 'solver_wall_time', sol.solve_time_seconds, 'elapsed', round(t1 - t0, 1), 'entries', len(sol.schedule_entries), 'operators_used', sol.operators_used)
finally:
    session.close()
