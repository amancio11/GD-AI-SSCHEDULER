...
try:
    ...
    for t in times:
        ...
        builder.TIMEOUT = t
        start=time.time(); sol=builder.build_and_solve('FINISH_BY_DATE', {}); duration=time.time()-start
        print('status', sol.status, 'time', round(duration,1), 'entries', len(sol.schedule_entries))
finally:
    session.close()
